...
> **⚠️ 訂正（component-design.md Rev.3による）**: `initializeTodayTodos`の呼び出し元は、下記「Server Component専用」という記述から変更されている。component-design.md Rev.3で、繰越・クリーンアップ（書き込みを伴う処理）をServer Componentのレンダリング経路から排除し、新設のServer Action `initializeToday(date)` からのみ呼ぶ方式に変更された（Server ComponentがSSR時に推測した日付で書き込みを行うと、日付の推測が外れた場合にデータ不整合を招く重大なバグがあったため）。**関数シグネチャ・内部ロジック自体は変更なし**。詳細はcomponent-design.md Rev.3を参照。
...
`YYYY-MM-DD` 形式の文字列を受け取るすべての関数（`getTodosByDate`, `initializeTodayTodos`, `createTodo`, `markStartNotificationSent`, `markEndNotificationSent`）は、共通ヘルパー `isValidDateString(date: string): boolean`（`lib/validation/rules.ts`、正規表現 `/^\d{4}-\d{2}-\d{2}$/` と実在日付チェック）で形式検証し、不正なら `ValidationError` を投げる（CLAUDE.md 4.2「日付文字列はYYYY-MM-DD形式のみ許可」を全関数で統一適用）。
...
| `initializeTodayTodos` | `(today: string) => Promise<Todo[]>` | ~~Server Component専用~~ **（訂正・component-design.md Rev.3）**: 新設のServer Action `initializeToday(date)` から、クライアントが渡す確実に正しい日付でのみ呼ぶ。継続Todoの自動繰越を同期実行し、30日クリーンアップは`after()`で非同期実行する |
...
1. ~~`initializeTodayTodos`をServer Componentから直接呼ぶ設計は...~~ **（訂正・component-design.md Rev.3）**: `initializeTodayTodos`はServer Componentから直接呼ばれる設計から、新設のServer Action `initializeToday(date)` 経由でクライアントが渡す日付でのみ呼ばれる設計に変更された（Server Component側が日付を推測して書き込みを行うと、推測が外れた場合にデータ不整合を招く重大なバグがあったため）。マウント時・`visibilitychange`時・複数タブから独立して呼ばれうる点は変わらないが、`last_carryover_date`ガードにより実害はない（冪等）。
...
3. ~~`initializeTodayTodos`をServer Actionとして実装し...不採用~~ **（訂正・component-design.md Rev.3で採用に変更）**: 初期表示のたびに追加ラウンドトリップが発生するというデメリットより、Server Component側の日付推測が書き込みの正しさを損なうリスクの方が重大と判断され、component-design.md Rev.3でこの案が採用された。追加ラウンドトリップは`initializeToday`が表示をブロックしない非同期呼び出しであるため、体感速度への影響は小さい。
